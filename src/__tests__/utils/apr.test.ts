...
describe('getFarmApr', () => {
  it(`returns null when parameters are missing`, () => {
    const { osRewardsApr, lpRewardsApr } = getFarmApr(null, null, null, null)
    expect(osRewardsApr).toBeNull()
    expect(lpRewardsApr).toEqual(0)
  })
  it(`returns null when APR is infinite`, () => {
    const { osRewardsApr, lpRewardsApr } = getFarmApr(BIG_ZERO, BIG_ZERO, BIG_ZERO, '')
    expect(osRewardsApr).toBeNull()
    expect(lpRewardsApr).toEqual(0)
  })
  it(`get the correct pool APR`, () => {
    const { osRewardsApr, lpRewardsApr } = getFarmApr(BIG_TEN, new BigNumber(1), new BigNumber(100000), '')
    expect(osRewardsApr).toEqual(4204800)
    expect(lpRewardsApr).toEqual(0)
  })
