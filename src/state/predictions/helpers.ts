...
export const transformBetResponse = (betResponse: BetResponse): Bet => {
  const bet = {
    ...
    claimedROSE: betResponse.claimedROSE ? parseFloat(betResponse.claimedROSE) : 0,
    claimedNetROSE: betResponse.claimedNetROSE ? parseFloat(betResponse.claimedNetROSE) : 0,
...
export const transformUserResponse = (userResponse: UserResponse): PredictionUser => {
  ...
  return {
    ...
    totalROSE: totalROSE ? parseFloat(totalROSE) : 0,
    totalROSEBull: totalROSEBull ? parseFloat(totalROSEBull) : 0,
    totalROSEBear: totalROSEBear ? parseFloat(totalROSEBear) : 0,
    totalBetsClaimed: numberOrNull(totalBetsClaimed),
    totalROSEClaimed: totalROSEClaimed ? parseFloat(totalROSEClaimed) : 0,
    winRate: winRate ? parseFloat(winRate) : 0,
    averageROSE: averageROSE ? parseFloat(averageROSE) : 0,
    netROSE: netROSE ? parseFloat(netROSE) : 0,
  }
}
...
export const getTotalWon = async (): Promise<number> => {
  ...
  const totalROSE = market.totalROSE ? parseFloat(market.totalROSE) : 0
  const totalROSETreasury = market.totalROSETreasury ? parseFloat(market.totalROSETreasury) : 0

  return Math.max(totalROSE - totalROSETreasury, 0)
}
