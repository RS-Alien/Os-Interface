...
interface OverviewResponse {
  oasisswapFactories: OasisswapFactory[]
}
...
const formatOasisswapFactoryResponse = (rawOasisswapFactory?: OasisswapFactory) => {
  if (rawOasisswapFactory) {
    return {
      totalTransactions: parseFloat(rawOasisswapFactory.totalTransactions),
      totalVolumeUSD: parseFloat(rawOasisswapFactory.totalVolumeUSD),
      totalLiquidityUSD: parseFloat(rawOasisswapFactory.totalLiquidityUSD),
    }
  }
  return null
}
...
const useFetchProtocolData = (): ProtocolFetchState => {
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      const overviewData = formatOasisswapFactoryResponse(data?.oasisswapFactories?.[0])
      const overviewData24 = formatOasisswapFactoryResponse(data24?.oasisswapFactories?.[0])
      const overviewData48 = formatOasisswapFactoryResponse(data48?.oasisswapFactories?.[0])
