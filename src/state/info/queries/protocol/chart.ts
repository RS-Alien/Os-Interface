...
import { OasisswapDayDatasResponse } from '../types'
import { fetchChartData, mapDayData } from '../helpers'

/**
 * Data for displaying Liquidity and Volume charts on Overview page
 */
const OASISSWAP_DAY_DATAS = gql`
  query overviewCharts($startTime: Int!, $skip: Int!) {
    oasisswapDayDatas(first: 1000, skip: $skip, where: { date_gt: $startTime }, orderBy: date, orderDirection: asc) {
      date
      dailyVolumeUSD
      totalLiquidityUSD
    }
...
const getOverviewChartData = async (skip: number): Promise<{ data?: ChartEntry[]; error: boolean }> => {
  try {
    const { oasisswapDayDatas } = await request<OasisswapDayDatasResponse>(INFO_CLIENT, OASISSWAP_DAY_DATAS, {
      startTime: PCS_V2_START,
      skip,
    })
    const data = oasisswapDayDatas.map(mapDayData)
