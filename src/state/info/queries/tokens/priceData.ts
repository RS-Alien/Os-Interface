...
/**
 * Price data for token and rose based on block number
 */
...
const fetchTokenPriceData = async (
  ...
  try {
    ...
    Object.keys(prices).forEach((priceKey) => {
      ...
      // if its ROSE price e.g. `b123` split('t')[1] will be undefined and skip ROSE price entry
      if (timestamp) {
        tokenPrices.push({
          timestamp,
    ...
    // Go through ROSE USD prices and calculate Token price based on it
    Object.keys(prices).forEach((priceKey) => {
      ...
      if (timestamp) {
        ...
        if (tokenPriceIndex >= 0) {
          const { derivedROSE } = tokenPrices[tokenPriceIndex]
          tokenPrices[tokenPriceIndex].priceUSD = parseFloat(prices[priceKey]?.rosePrice ?? 0) * derivedROSE
        }
      }
    })
