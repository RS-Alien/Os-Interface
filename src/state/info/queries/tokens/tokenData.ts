...
import { useRosePrices } from 'views/Info/hooks/useRosePrices'

interface TokenFields {
  id: string
  symbol: string
  name: string
  derivedROSE: string // Price in ROSE per token
...
interface FormattedTokenFields
  extends Omit<TokenFields, 'derivedROSE' | 'derivedUSD' | 'tradeVolumeUSD' | 'totalTransactions' | 'totalLiquidity'> {
  derivedROSE: number
  derivedUSD: number
...
const parseTokenData = (tokens?: TokenFields[]) => {
  ...
  return tokens.reduce((accum: { [address: string]: FormattedTokenFields }, tokenData) => {
    const { derivedROSE, derivedUSD, tradeVolumeUSD, totalTransactions, totalLiquidity } = tokenData
    accum[tokenData.id] = {
      ...tokenData,
      derivedROSE: parseFloat(derivedROSE),
...
const useFetchedTokenDatas = (tokenAddresses: string[]): TokenDatas => {
  ...
  const rosePrices = useRosePrices()
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      if (error) {
        ...
        const formatted = tokenAddresses.reduce((accum: { [address: string]: TokenData }, address) => {
          ...
          const priceUSD = current ? current.derivedROSE * rosePrices.current : 0
          const priceUSDOneDay = oneDay ? oneDay.derivedROSE * rosePrices.oneDay : 0
          const priceUSDWeek = week ? week.derivedROSE * rosePrices.week : 0
    ...
    if (tokenAddresses.length > 0 && allBlocksAvailable && !blockError && rosePrices) {
      fetch()
    }
  }, [tokenAddresses, block24h, block48h, block7d, block14d, blockError, rosePrices])
