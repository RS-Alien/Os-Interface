...
const fetchOsRewardsForTickets = async (
  winningTickets: LotteryTicket[],
): Promise<{ ticketsWithUnclaimedRewards: LotteryTicket[]; osTotal: BigNumber }> => {
  ...
  try {
    const osRewards = await multicallv2(lotteryV2Abi, calls)

    const osTotal = osRewards.reduce((accum: BigNumber, osReward: ethers.BigNumber[]) => {
      return accum.plus(new BigNumber(osReward[0].toString()))
    }, BIG_ZERO)

    const ticketsWithUnclaimedRewards = winningTickets.map((winningTicket, index) => {
      return { ...winningTicket, osReward: osRewards[index] }
    })
    return { ticketsWithUnclaimedRewards, osTotal }
  } catch (error) {
    console.error(error)
    return { ticketsWithUnclaimedRewards: null, osTotal: null }
  }
}
...
export const getWinningTickets = async (
  ...
  if (unclaimedWinningTickets.length > 0) {
    const { ticketsWithUnclaimedRewards, osTotal } = await fetchOsRewardsForTickets(unclaimedWinningTickets)
    return { ticketsWithUnclaimedRewards, allWinningTickets, osTotal, roundId }
  }

  if (allWinningTickets.length > 0) {
    return { ticketsWithUnclaimedRewards: null, allWinningTickets, osTotal: null, roundId }
  }
