...
// Note: OasisswapBunny specific
export const useFetchByBunnyIdAndUpdate = (bunnyId: string) => {
  const dispatch = useAppDispatch()

  const { latestOasisswapBunniesUpdateAt, isUpdatingOasisswapBunnies } = useSelector(
  ...
  const allOasisswapBunnies = useNftsFromCollection(oasisswapBunniesAddress)
  const allExistingPBTokenIds = allOasisswapBunnies ? allOasisswapBunnies.map((nft) => nft.tokenId) : []
  ...
  const fetchMoreOasisswapBunnies = (orderDirection: 'asc' | 'desc') => {
    dispatch(
  ...
  return { isUpdatingOasisswapBunnies, latestOasisswapBunniesUpdateAt, fetchMoreOasisswapBunnies }
}
...
export const useGetAllBunniesByBunnyId = (bunnyId: string) => {
  const nfts: NftToken[] = useSelector((state: State) => state.nftMarket.data.nfts[oasisswapBunniesAddress])
