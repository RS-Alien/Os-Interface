...
import {
  ...
  getOasisswapBunniesAttributesField,
...
const initialState: State = {
  ...
  data: {
    ...
    activityFilters: {},
    loadingState: {
      isUpdatingOasisswapBunnies: false,
      latestOasisswapBunniesUpdateAt: 0,
    },
    users: {},
    user: {
...
>('nft/fetchNftsFromCollections', async ({ collectionAddress, page, size }) => {
  try {
    if (collectionAddress === oasisswapBunniesAddress) {
      // OasisswapBunnies don't need to pre-fetch "all nfts" from the collection
...
/**
 * This action keeps data on the individual OasisswapBunny page up-to-date. Operation is a twofold
  ...
  async ({ bunnyId, existingTokensWithBunnyId, allExistingPBTokenIds, existingMetadata, orderDirection }) => {
    try {
      ...
      const [updatedNfts, updatedNftsMarket] = await Promise.all([
        getNftsFromCollectionApi(oasisswapBunniesAddress),
        getMarketDataForTokenIds(oasisswapBunniesAddress, allExistingPBTokenIds),
      ])
      ...
      const updatedTokens = updatedNftsMarket.map((marketData) => {
        ...
        const attributes = getOasisswapBunniesAttributesField(marketData.otherId)
      ...
      if (!existingMetadata) {
        newNfts = await getNftsFromCollectionApi(oasisswapBunniesAddress)
      }
      ...
      console.error(`Failed to update OasisswapBunnies NFTs`, error)
      return []
    }
  },
)
...
export const NftMarket = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(fetchNewPBAndUpdateExisting.pending, (state) => {
      state.data.loadingState.isUpdatingOasisswapBunnies = true
    })
    builder.addCase(fetchNewPBAndUpdateExisting.fulfilled, (state, action) => {
      if (action.payload.length > 0) {
        state.data.nfts[oasisswapBunniesAddress] = action.payload
      }
      state.data.loadingState.isUpdatingOasisswapBunnies = false
      state.data.loadingState.latestOasisswapBunniesUpdateAt = Date.now()
    })
