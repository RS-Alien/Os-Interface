...
/**
 * Fetch market data for OasisswapBunnies NFTs by bunny id using the Subgraph
...
export const getNftsByBunnyIdSg = async (
  ...
  try {
    ...
    const res = await request(
      ...
      `,
      {
        collectionAddress: oasisswapBunniesAddress.toLowerCase(),
        where,
        orderDirection,
      },
    )
...
export const getAllOasisswapBunniesLowestPrice = async (bunnyIds: string[]): Promise<Record<string, number>> => {
  try {
    const singleOasisswapBunnySubQueries = bunnyIds.map(
      (
        bunnyId,
    ...
    const rawResponse: Record<string, { currentAskPrice: string }[]> = await request(
      GRAPH_API_NFTMARKET,
      gql`
        query getAllOasisswapBunniesLowestPrice {
          ${singleOasisswapBunnySubQueries}
        }
      `,
    )
    ...
    console.error('Failed to fetch OasisswapBunnies lowest prices', error)
    return {}
  }
}

export const getAllOasisswapBunniesRecentUpdatedAt = async (bunnyIds: string[]): Promise<Record<string, number>> => {
  try {
    const singleOasisswapBunnySubQueries = bunnyIds.map(
      (
        bunnyId,
    ...
    console.error('Failed to fetch OasisswapBunnies latest market updates', error)
    return {}
  }
}
...
export const getMetadataWithFallback = (apiMetadata: ApiResponseCollectionTokens['data'], bunnyId: string) => {
  ...
  return (
    apiMetadata[bunnyId] ?? {
      name: '',
      description: '',
      collection: { name: 'Oasisswap Bunnies' },
      image: {
        original: '',
        thumbnail: '',
      },
...
export const getOasisswapBunniesAttributesField = (bunnyId: string) => {
  ...
  const walletNftsWithMarketData = walletNfts.map((walletNft) => {
    ...
    return (
      marketData ?? {
        ...
        latestTradedPriceInROSE: null,
        tradeVolumeROSE: null,
        totalTrades: null,
        otherId: null,
      }
    )
