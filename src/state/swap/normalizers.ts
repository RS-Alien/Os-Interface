...
export const normalizeDerivedChartData = (data: any) => {
  if (!data?.token0DerivedRose || data?.token0DerivedRose.length === 0) {
    return []
  }
  return data?.token0DerivedRose.reduce((acc, token0DerivedRoseEntry) => {
    const token1DerivedRoseEntry = data?.token1DerivedRose?.find(
      (entry) => entry.timestamp === token0DerivedRoseEntry.timestamp,
    )
    if (!token1DerivedRoseEntry) {
      return acc
    }
    return [
      ...acc,
      {
        time: parseInt(token0DerivedRoseEntry.timestamp, 10),
        token0Id: token0DerivedRoseEntry.tokenAddress,
        token1Id: token1DerivedRoseEntry.tokenAddress,
        token0DerivedROSE: token0DerivedRoseEntry.derivedROSE,
        token1DerivedROSE: token1DerivedRoseEntry.derivedROSE,
      },
    ]
  }, [])
}
...
export const normalizeDerivedPairDataByActiveToken = ({
  ...
  pairData?.map((pairPrice) => ({
    ...
    value:
      activeToken === pairPrice?.token0Id
        ? pairPrice.token0DerivedROSE / pairPrice.token1DerivedROSE
        : pairPrice.token1DerivedROSE / pairPrice.token0DerivedROSE,
  }))
