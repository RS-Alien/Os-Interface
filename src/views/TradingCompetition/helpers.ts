...
export const useCompetitionOsRewards = (userOsReward: ReactText) => {
  const osAsBigNumber = new BigNumber(userOsReward as string)
  const osBalance = getBalanceNumber(osAsBigNumber)
  const osPriceUsdt = usePriceOsUsdt()
  return {
    osReward: osBalance,
    dollarValueOfOsReward: osPriceUsdt.gt(0) ? osBalance * osPriceUsdt.toNumber() : null,
  }
}
