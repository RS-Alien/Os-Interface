...
import { Image, Flex, Text, Td, IconButton, Link, OpenNewIcon, useMatchBreakpoints, useModal } from '@oasisswap/uikit'
import { Link as RouterLink } from 'react-router-dom'
import { Activity, NftToken } from 'state/nftMarket/types'
import { Price } from '@oasisswap/sdk'
...
import { nftsBaseUrl, oasisswapBunniesAddress } from '../../constants'
...
const ActivityRow: React.FC<ActivityRowProps> = ({
  ...
  const [onPresentMobileModal] = useModal(
    <MobileModal
      ...
      roseUsdtPrice={roseUsdtPrice}
      isUserActivity={isUserActivity}
    />,
  )
  const isPBCollection = nft ? nft.collectionAddress.toLowerCase() === oasisswapBunniesAddress.toLowerCase() : false
  const tokenId =
    nft && isPBCollection
  ...
  return (
    <tr {...((isXs || isSm) && { onClick: onPresentMobileModal })}>
      ...
      <Td>
        ...
        {isXs || isSm ? <ActivityPrice price={priceAsFloat} roseUsdtPrice={roseUsdtPrice} /> : null}
      </Td>
      {isXs || isSm ? null : (
        <>
          <Td>
            <ActivityPrice price={priceAsFloat} roseUsdtPrice={roseUsdtPrice} />
          </Td>
          {isUserActivity ? (
            <Td>
