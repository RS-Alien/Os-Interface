...
import { Flex, Grid, Box, Text, Button, BinanceIcon, ErrorIcon, useTooltip } from '@oasisswap/uikit'
...
import { GreyedOutContainer, RoseAmountCell, RightAlignedInput, FeeAmountCell } from './styles'
...
const SetPriceStage: React.FC<SetPriceStageProps> = ({
  ...
  const rosePrice = useROSEUsdtPrice()
  const priceAsFloat = parseFloat(price)
  const priceInUsd = multiplyPriceByAmount(rosePrice, priceAsFloat)
  ...
  const { tooltip, tooltipVisible, targetRef } = useTooltip(
    <>
      <Text>
        {t(
          'When selling NFTs from this collection, a portion of the ROSE paid will be diverted before reaching the seller:',
        )}
      </Text>
      {creatorFeeAsNumber > 0 && (
        <Text>{t('%percentage%% royalties to the collection owner', { percentage: creatorFee })}</Text>
      )}
      <Text>{t('%percentage%% trading fee will be used to buy & burn OS', { percentage: tradingFee })}</Text>
    </>,
    { placement: 'auto' },
  )
    ...
    <>
      ...
      <GreyedOutContainer>
        ...
        {priceIsOutOfRange && (
          <Text fontSize="12px" color="failure">
            {t('Allowed price range is between %minPrice% and %maxPrice% WROSE', {
              minPrice: MIN_PRICE,
              maxPrice: MAX_PRICE,
            })}
          </Text>
        ...
        <Flex justifyContent="space-between" alignItems="center" mt="16px">
          ...
          <FeeAmountCell roseAmount={priceAsFloat} creatorFee={creatorFeeAsNumber} tradingFee={tradingFeeAsNumber} />
        </Flex>
        {lowestPrice && (
          <Flex justifyContent="space-between" alignItems="center" mt="16px">
            <Text small color="textSubtle">
            ...
            <RoseAmountCell roseAmount={lowestPrice} />
          </Flex>
        )}
      </GreyedOutContainer>
      <Grid gridTemplateColumns="32px 1fr" p="16px" maxWidth="360px">
        ...
        <Box>
          ...
          <Text small color="textSubtle">
            {t('Sales are in WROSE. You can swap WROSE to ROSE 1:1 for free with OasisSwap.')}
          </Text>
        </Box>
      </Grid>
      <Divider />
