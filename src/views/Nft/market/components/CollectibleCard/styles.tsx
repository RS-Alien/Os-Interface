...
interface ROSEAmountLabelProps extends FlexProps {
  amount: number
}

export const ROSEAmountLabel: React.FC<ROSEAmountLabelProps> = ({ amount, ...props }) => (
...
export const CostLabel: React.FC<CostLabelProps> = ({ cost, roseUsdtPrice, ...props }) => {
  const priceInUsd = multiplyPriceByAmount(roseUsdtPrice, cost)
...
export const LowestPriceMetaRow = ({ lowestPrice, isFetching, roseUsdtPrice }: LowestPriceMetaRowProps) => {
  const { t } = useTranslation()
  ...
  return (
    <MetaRow title={t('Lowest price')}>
      {isFetching ? (
        ...
        <CostLabel cost={lowestPrice} roseUsdtPrice={roseUsdtPrice} />
      )}
    </MetaRow>
  )
}
