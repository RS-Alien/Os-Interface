...
import { Button, Grid, Text, Flex, Box, BinanceIcon, useModal, Skeleton } from '@oasisswap/uikit'
...
const Row: React.FC<RowProps> = ({ t, nft, roseUsdtPrice, account }) => {
  const priceInUsd = multiplyPriceByAmount(roseUsdtPrice, parseFloat(nft.marketData.currentAskPrice))
...
const ForSaleTableRow: React.FC<ForSaleTableRowsProps> = ({ nftsForSale }) => {
  ...
  return (
    <OwnersTableRow>
      {nftsForSale.map((nft) => (
        <Row key={nft.tokenId} t={t} nft={nft} roseUsdtPrice={roseUsdtPrice} account={account} />
      ))}
    </OwnersTableRow>
  )
}
