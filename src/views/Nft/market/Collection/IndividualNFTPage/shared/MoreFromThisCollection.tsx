...
import { ArrowBackIcon, ArrowForwardIcon, Box, IconButton, Text, Flex, useMatchBreakpoints } from '@oasisswap/uikit'
...
const MoreFromThisCollection: React.FC<MoreFromThisCollectionProps> = ({
  ...
  const allOasisswapBunnyNfts = useAllOasisswapBunnyNfts(collectionAddress)
  const collectionNfts = useNftsFromCollection(collectionAddress)

  const isPBCollection = isAddress(collectionAddress) === oasisswapBunniesAddress
  ...
  let nftsToShow = useMemo(() => {
    return shuffle(
      allOasisswapBunnyNfts
        ? allOasisswapBunnyNfts.filter((nft) => nft.name !== currentTokenName)
        : collectionNfts?.filter((nft) => nft.name !== currentTokenName && nft.marketData?.isTradable),
    )
  }, [allOasisswapBunnyNfts, collectionNfts, currentTokenName])
  ...
  if (isPBCollection) {
    // OasisswapBunnies should display 1 card per bunny id
