...
const Header: React.FC<HeaderProps> = ({ collection }) => {
  ...
  const { totalSupply, numberTokensListed, totalVolumeROSE, banner, avatar } = collection
  const { t } = useTranslation()
  const { pathname, hash } = useLocation()

  const volume = totalVolumeROSE
    ? parseFloat(totalVolumeROSE).toLocaleString(undefined, {
      ...
      <MarketPageHeader>
        ...
        >
          <StatBox>
            ...
            <StatBoxItem title={t('Vol. (%symbol%)', { symbol: 'ROSE' })} stat={volume} />
          </StatBox>
        </MarketPageTitle>
      </MarketPageHeader>
      <Container>
