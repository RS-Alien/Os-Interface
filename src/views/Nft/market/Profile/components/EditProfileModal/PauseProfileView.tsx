...
import { AutoRenewIcon, Button, Checkbox, Flex, InjectedModalProps, Text } from '@oasisswap/uikit'
...
const PauseProfilePage: React.FC<PauseProfilePageProps> = ({ onDismiss }) => {
  ...
  const {
    costs: { numberOsToReactivate },
  } = useGetProfileCosts()
  const { t } = useTranslation()
  const oasisswapProfileContract = useProfileContract()
  ...
  const handleDeactivateProfile = async () => {
    const tx = await callWithGasPrice(oasisswapProfileContract, 'pauseProfile')
  ...
  return (
    <>
      ...
      <Text as="p" color="textSubtle" mb="24px">
        {t('Cost to reactivate in the future: %cost% OS', { cost: formatBigNumber(numberOsToReactivate) })}
      </Text>
