...
import { Button, Box, InjectedModalProps, Text } from '@oasisswap/uikit'
  ...
  const { isApproving, isApproved, isConfirmed, isConfirming, handleApprove, handleConfirm } =
    useApproveConfirmTransaction({
      onApprove: () => {
        ...
        return callWithGasPrice(contract, 'approve', [getOasisswapProfileAddress(), selectedNft.tokenId])
      },
      onConfirm: () => {
        if (!profile.isActive) {
    ...
    <>
      ...
      {nfts.length === 0 && (
        <>
          ...
          <Text as="p" color="textSubtle" mb="24px">
            {t('Make sure you have a Oasisswap Collectible in your wallet and try again!')}
          </Text>
        </>
      )}
