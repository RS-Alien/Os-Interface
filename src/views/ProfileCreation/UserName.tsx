...
const UserName: React.FC = () => {
  ...
  const { teamId, selectedNft, userName, actions, minimumOsRequired, allowance } = useProfileCreation()
  ...
  const hasMinimumOsRequired = fetchStatus === FetchStatus.SUCCESS && osBalance.gte(REGISTER_COST)
  const [onPresentConfirmProfileCreation] = useModal(
    <ConfirmProfileCreationModal
      ...
      account={account}
      teamId={teamId}
      minimumOsRequired={minimumOsRequired}
      allowance={allowance}
    />,
    false,
  )
    ...
    <>
      ...
      {!hasMinimumOsRequired && (
        <Text color="failure" mt="16px">
          {t('A minimum of %num% OS is required', { num: formatUnits(REGISTER_COST) })}
        </Text>
      )}
    </>
  )
