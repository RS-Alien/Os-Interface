...
import { Token } from '@oasisswap/sdk'
import { Ifo, PoolIds } from 'config/constants/types'
import tokens from 'config/constants/tokens'
import { osRoseLpToken } from 'config/constants/ifo'
...
const CommitTokenSection: React.FC<TokenSectionProps & { commitToken: Token }> = ({ commitToken, ...props }) => {
  if (commitToken.equals(osRoseLpToken)) {
    return <TokenSection primaryToken={tokens.os} secondaryToken={tokens.wrose} {...props} />
  }
...
const IfoCardTokens: React.FC<IfoCardTokensProps> = ({
  ...
  const { targetRef, tooltip, tooltipVisible } = useTooltip(
    t(
      'Sorry, you didn’t contribute enough OS to meet the minimum threshold. You didn’t buy anything in this sale, but you can still reclaim your OS.',
    ),
    { placement: 'bottom' },
  )
  ...
  const renderTokenSection = () => {
    ...
    if (account && !hasProfile) {
      if (publicIfoData.status === 'finished') {
        return <Text textAlign="center">{t('Activate OasisSwap Profile to take part in next IFO‘s!')}</Text>
      }
      return <Text textAlign="center">{t('You need an active OasisSwap Profile to take part in an IFO!')}</Text>
    }
