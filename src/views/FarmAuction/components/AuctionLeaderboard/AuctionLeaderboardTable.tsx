...
interface LeaderboardRowProps {
  bidder: Bidder
  osPriceUsdt: BigNumber
  isMobile: boolean
}

const LeaderboardRow: React.FC<LeaderboardRowProps> = ({ bidder, osPriceUsdt, isMobile }) => {
        ...
        <Flex flexDirection="column" width="100%" justifyContent="flex-end" pr={[null, null, '24px']}>
          ...
          {osPriceUsdt.gt(0) ? (
            <Text fontSize="12px" color="textSubtle" textAlign="right">
              ~${getBalanceNumber(amount.times(osPriceUsdt)).toLocaleString('en', { maximumFractionDigits: 0 })}
            </Text>
          ) : (
    ...
    <Box>
      <LeaderboardContainer>
        ...
        {bidders.slice(0, visibleBidders).map((bidder) => (
          <LeaderboardRow key={bidder.account} bidder={bidder} osPriceUsdt={osPriceUsdt} isMobile={isMobile} />
        ))}
      </LeaderboardContainer>
