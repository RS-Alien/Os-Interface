...
import { Modal, Text, Flex, BalanceInput, Box, Button, OasisswapRoundIcon } from '@oasisswap/uikit'
...
const PlaceBidModal: React.FC<PlaceBidModalProps> = ({
  ...
  useEffect(() => {
    ...
    if (fetchStatus === FetchStatus.SUCCESS && userOsBalance.lt(bid)) {
  ...
  const { isApproving, isApproved, isConfirmed, isConfirming, handleApprove, handleConfirm } =
    useApproveConfirmTransaction({
      ...
      onApprove: () => {
        return callWithGasPrice(osContract, 'approve', [farmAuctionContract.address, ethers.constants.MaxUint256])
      },
    ...
    <StyledModal title={t('Place a Bid')} onDismiss={onDismiss} headerBackground={theme.colors.gradients.cardHeader}>
      ...
      <InnerContent>
        <Flex justifyContent="space-between" alignItems="center" pb="8px">
          ...
          <Flex>
            <OasisswapRoundIcon width="24px" height="24px" mr="4px" />
            <Text bold>OS</Text>
          </Flex>
        </Flex>
        {isFirstBid && (
        ...
        <BalanceInput
          ...
          currencyValue={
            osPriceUsdt.gt(0) &&
            `~${bid ? osPriceUsdt.times(new BigNumber(bid)).toNumber().toLocaleString() : '0.00'} USD`
          }
        />
