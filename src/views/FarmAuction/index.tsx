...
import { Button, Heading, Text, Flex, Link, Breadcrumbs } from '@oasisswap/uikit'
...
import AuctionOsBurn from './components/AuctionOsBurn'
...
const FarmAuction = () => {
  ...
  const OS_BURN_BG_LIGHT = 'radial-gradient(50% 79.31% at 50% 50%, #FAF9FA 0%, #EAECF4 100%)'
  const OS_BURN_TOP_FILL_LIGHT = 'radial-gradient(ellipse at bottom, #f0f1f6, #EAECF4)'
  const OS_BURN_BG_DARK = 'radial-gradient(103.12% 50% at 50% 50%, #152534 0%, #191326 100%)'
  const OS_BURN_TOP_FILL_DARK = '#191326'
  ...
  return (
    <>
        ...
        <Flex flexDirection={['column-reverse', null, 'row']}>
          <Left>
            ...
            <Text color="textSubtle" mb="24px">
              {t('Each week, qualifying projects can bid OS for the right to host a 7-day Farm on OasisSwap.')}
            </Text>
      ...
      <>
        ...
        <PageSection
          background={theme.isDark ? OS_BURN_BG_DARK : OS_BURN_BG_LIGHT}
          index={2}
          innerProps={{ style: { width: '100%' } }}
          dividerPosition="top"
          dividerFill={{ light: OS_BURN_TOP_FILL_LIGHT, dark: OS_BURN_TOP_FILL_DARK }}
        >
          <AuctionOsBurn />
        </PageSection>
        <PageSection
          ...
          dividerFill={{ light: OS_BURN_BG_LIGHT, dark: OS_BURN_BG_DARK }}
          concaveDivider
          index={3}
          dividerPosition="top"
        >
