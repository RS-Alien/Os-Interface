...
import { SaleStatusEnum } from 'views/OasisswapSquad/types'
...
const BuyTicketsModal: React.FC<BuyTicketsModalProps> = ({
  ...
  const isOsBalanceInsufficient = osBalance.lt(pricePerTicket)
  const maxBuyTickets = Math.min(osBalance.div(pricePerTicket).toNumber(), remainingTickets)
        ...
        <Flex flexDirection="column">
          <Box px="16px">
            ...
            <Flex mb="8px" justifyContent="space-between">
              ...
              <Text font-size="14px">{formatBigNumber(pricePerTicket, 0)} OS</Text>
            </Flex>
            <Flex mb="8px" justifyContent="space-between">
              <Text font-size="14px" color="textSubtle">
                {t('Your OS Balance')}
              </Text>
              <Text font-size="14px" color={isOsBalanceInsufficient ? 'failure' : 'text'}>
                {formatBigNumber(osBalance, 3)} OS
              </Text>
            </Flex>
            <Flex
              mb="8px"
            ...
            <Flex mb="25px" justifyContent="space-between">
              ...
              <Text font-size="14px" bold>
                {formatBigNumber(totalCost, 0)} OS
              </Text>
            </Flex>
          </Box>
          <Flex
          ...
          <Box px="16px">
            <Button
              disabled={isOsBalanceInsufficient}
              onClick={() => buyTicketCallBack({ ticketsNumber })}
              width="100%"
            >
              {isOsBalanceInsufficient ? t('Insufficient Balance') : t('Confirm')}
            </Button>
          </Box>
        </Flex>
      </ModalBody>
