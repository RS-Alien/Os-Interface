...
import { Button, useModal } from '@oasisswap/uikit'
import { ContextApi } from 'contexts/Localization/types'
import { ethers, BigNumber } from 'ethers'
import useApproveConfirmTransaction from 'hooks/useApproveConfirmTransaction'
import { useCallWithGasPrice } from 'hooks/useCallWithGasPrice'
import { useOs, useNftSaleContract } from 'hooks/useContract'
import useToast from 'hooks/useToast'
import { DefaultTheme } from 'styled-components'
import { ethersToBigNumber } from 'utils/bigNumber'
import { OasisswapSquadContext } from 'views/OasisswapSquad/context'
...
const BuyTicketsButtons: React.FC<BuyTicketsProps> = ({
  ...
  const { isUserEnabled, setIsUserEnabled } = useContext(OasisswapSquadContext)
  ...
  const { isApproving, isApproved, isConfirming, handleApprove, handleConfirm, hasApproveFailed, hasConfirmFailed } =
    useApproveConfirmTransaction({
      onRequiresApproval: async () => {
        try {
          const response = await osContract.allowance(account, nftSaleContract.address)
      ...
      onApprove: () => {
        return callWithGasPrice(osContract, 'approve', [nftSaleContract.address, ethers.constants.MaxUint256])
      },
  ...
  const [onPresentEnableModal, onDismissEnableModal] = useModal(
    <ConfirmModal
      ...
      loadingText={t('Please enable OS spending in your wallet')}
