...
import { Text, Flex, Box, CloseIcon, IconButton, useMatchBreakpoints } from '@oasisswap/uikit'
...
const PhishingWarningBanner: React.FC = () => {
  ...
  const warningText = t("please make sure you're visiting https://oasisswap.io - check the URL carefully.")
  const warningTextAsParts = warningText.split(/(https:\/\/oasisswap.io)/g)
    ...
    <>
      ...
      {warningTextAsParts.map((text, i) => (
        <Text
          ...
          small
          as="span"
          bold={text === 'https://oasisswap.io'}
          color={text === 'https://oasisswap.io' ? '#FFFFFF' : '#BDC2C4'}
        >
          {text}
        </Text>
      ))}
